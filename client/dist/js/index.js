...
const toggleAuthModal = () => {
  const loginDialog = document.getElementById('js-modal-login');
  const registerDialog = document.getElementById('js-modal-register');
  const toggleLinks = document.getElementsByClassName('link');
  [...toggleLinks].forEach((link) => {
    link.addEventListener('click', (e) => {
      e.preventDefault();
      loginDialog.classList.toggle('show');
      registerDialog.classList.toggle('show');
    });
  })
};

const serializeForm = (form) => {
  return $(form).serializeArray().reduce((res, el) => {
...
const setupAuthEvent = () => {
  ...
  const registerForm = registerDialog.querySelector('#register-form');
  registerForm.addEventListener('submit', onRegisterUser);

  if (!isAuth) loginDialog.classList.add('show');
};
...
const addToCart = (p) => {
  const inCart = cart.find((ci) => ci.product._id === p._id);
  if (inCart) {
...
const setupFilterBlock = () => {
  ...
  filterContainer.addEventListener('click', (e) => {
    ...
    if (container) {
      container.closest('.filter-block').classList.toggle('open');
      const icl = container.querySelector('.i').classList;
